<?php
    ...
    class CurrentUserRepository {
        ...
        public static function getUserOpenTickets() {
            ...
            $db = Database::getConnection();
            $session_token = $_COOKIE['sessionToken'];
            $statement = $db->prepare("SELECT * FROM sessions INNER JOIN tickets ON sessions.user_id = tickets.ticket_creator INNER JOIN ticket_status ON tickets.ticket_status = ticket_status.status_id WHERE `session_token`=:session_token AND NOT tickets.`ticket_is_complete`=1 ORDER BY tickets.`ticket_id`;");
            $statement->bindParam(":session_token", $session_token);
            $statement->execute();
        ...
        public static function getUserOpenAssignedTickets() {
            $db = Database::getConnection();
            $session_token = $_COOKIE['sessionToken'];
            $statement = $db->prepare("SELECT * FROM sessions INNER JOIN tickets ON sessions.user_id = tickets.ticket_technician_owner INNER JOIN ticket_status ON tickets.ticket_status = ticket_status.status_id WHERE `session_token`=:session_token AND `ticket_is_complete`=0;");
            $statement->bindParam(":session_token", $session_token);
            $statement->execute();
