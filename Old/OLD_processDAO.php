<?php
    ...
    function getProcess($process_id) {
        ...
        try {
            ...
            $statement = $db->prepare("SELECT * FROM submitted_process AS process WHERE `submitted_process_id`=:process_id;");
            $statement->bindParam(":process_id", $process_id);
            $statement->execute();
            $result = $statement->fetch(PDO::FETCH_OBJ);
            return ($result);
    ...
    function getProcessWithUserDetails($process_id) {
        try {
            $db = $GLOBALS['db'];
